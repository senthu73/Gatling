...
class test5 extends Simulation {
  ...
  val httpConf = http.baseUrl(url = "https://prf.doterra.com/")
    ...
    .acceptCharsetHeader("UTF-8")
        ...
        val sessionHeaders = Map("Authorization" -> s"Bearer  ${clientSecret}" )
//        val sessionHeaders = Map("Authorization" -> "Bearer "+${secret} )
  ...
  def init()={
    ...
    exec(http(requestName = "login")
      ...
      .header("Authorization", s"Bearer   + ${clientSecret}")
  ...
  val scn = scenario(scenarioName = "oneTimeOrder")
    ...
    .exec( session => {
      ...
      clientId = session("username").as[String]
      clientPwd = session("password").as[String]

      println("member:" + clientId)
//      clientSecret = session(CreateJWTtokenRequest.loginToken(clientId, clientPwd)).as[String]
      clientSecret =  CreateJWTtokenRequest.loginToken(clientId, clientPwd)
      println("*Authorization Bearer 1" + clientSecret)
      session


    })

        .exec( session =>{ println(s"*Authorization Bearer 2  +  ${clientSecret}")
